...
readFileLines(file)
    .then((lines) => {
       ...
       //We have numbers and special characters now, in input.
        const partNumbers = [];

        // Fetch numbers
        for (const numberData of allNumbers) {
            const { number, line, indexes } = numberData
            let isPartNumber = false;
            for (const index of indexes) {
                // For every digits , we check the adjacents characters
                const adjacentCharacters = adjacentOfchar(grid, line, index)
                // If one special char at least, we have a 'part number'
        ...
        const sumOfPartNumbers = partNumbers.reduce((acc, curr) => acc + curr, 0)
